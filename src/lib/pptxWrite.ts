// Pure-JS PowerPoint (.pptx) writer, shared in spirit with the presenter's
// src/main/pptxWrite.ts — this copy adds JPEG frames and image de-duplication,
// because a service shared from a phone has to stay small enough to send.
//
...
const slideRels = (media: number, ext: string): string =>
  XML_DECL +
  `<Relationships xmlns="${PKG_REL}">` +
  `<Relationship Id="rId1" Type="${REL}/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>` +
  `<Relationship Id="rId2" Type="${REL}/image" Target="../media/image${media}.${ext}"/>` +
  '</Relationships>'

/** One captured frame: the encoded bytes and which encoder won. */
...
/**
 * Pack a sequence of captured frames (one per slide, in order) into a .pptx.
 * Each frame becomes a full-bleed 16:9 slide.
 *
 * Identical frames are stored ONCE and referenced by every slide that shows
 * them. A worship set repeats: the refrain comes back between every stanza, a
 * section card sits in front of each song, and a blank is a blank. Those repeats
 * are byte-identical captures, and storing each again is how a deck that could
 * be three megabytes becomes eight.
 */
export function buildPptx(frames: PptxImage[]): Uint8Array {
  ...
  // Key on the bytes themselves. A capture of the same slide is deterministic,
  // so equal content really is equal bytes — no hashing needed beyond this.
  const seen = new Map<string, { index: number; ext: string }>()
  let mediaCount = 0
  for (let i = 1; i <= n; i++) {
    const f = frames[i - 1]
    const key = `${f.ext}:${f.bytes.length}:${fingerprint(f.bytes)}`
    let at = seen.get(key)
    if (!at) {
      at = { index: ++mediaCount, ext: f.ext }
      seen.set(key, at)
      files[`ppt/media/image${at.index}.${at.ext}`] = f.bytes
    }
    files[`ppt/slides/slide${i}.xml`] = strToU8(slideXml)
    files[`ppt/slides/_rels/slide${i}.xml.rels`] = strToU8(slideRels(at.index, at.ext))
  }
  files['[Content_Types].xml'] = strToU8(contentTypes(n))

  // The media is already compressed; a low level keeps the XML small without
  // burning CPU trying to shrink JPEG.
  return zipSync(files, { level: 4 })
}

/** Cheap content fingerprint — enough to tell two captures apart when their
 *  lengths already match. Collisions would have to agree on length AND every
 *  sampled byte, which for our own renders does not happen. */
function fingerprint(b: Uint8Array): string {
  ...
  for (let i = 0; i < b.length; i += step) {
    ...
    h2 = Math.imul(h2 + b[i] + i, 0x85ebca6b)
  }
  return `${h1 >>> 0}.${h2 >>> 0}`
}
