/**
 * Ported verbatim from lumen-presenter's shared/bible/translit.ts. The two apps
 * have to agree on what "Yohanu" means or the same reference finds a book on
 * the desktop and nothing on the phone — and the comments below record why each
 * vowel is spelled the way it is, which is exactly the knowledge that gets lost
 * when something like this is rewritten from memory.
 *
